...
Help opens on the right, occur on top, the shell at the bottom, and
the editing area in the middle stays put.  One command detaches a
side window into a normal one and puts it back.
...
Once the package is on MELPA, install it with the built in package
...
A minimal, working setup — the demo above runs on exactly this:
...
- =auto-side-windows-display-buffer-on-side=: Manually force the current buffer to be displayed in a side window on the specified side, overriding normal rules. Compared to the commands above, it asks for the side in the minibuffer.
...
The face remapping recipe that used to live here grew into a package
of its own: [[https://github.com/MArpogaus/window-box][window-box]] draws a rectangular box around a window and
leaves the mode line and the header line alone.  Boxing everything
that lands in a side window is one hook:
