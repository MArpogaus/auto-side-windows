...
=auto-side-windows-drag-slot= does the same with the mouse.  It is the
one key this package binds, on the header line of a side window:

#+begin_src emacs-lisp
  ;; what auto-side-windows-mode-map holds
  "<header-line> <drag-mouse-1>"
#+end_src

Press mouse-1 on the header line of a side window, drag to another side
window of the same side and let go, and the two buffers change places.
A drag that ends anywhere else does nothing, because a slot belongs to
one side.  A side window needs a header line for this; the window
parameters of a side decide that.
