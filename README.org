...
slot zero.  The slots and the sizes of the windows stay as they are,
and point follows the buffer, so a repeated key keeps moving the same
one.  The package binds no key for them; bind what you like.
