...
With =auto-side-windows-remember-sizes= at its default of t, a side
window you resize keeps its size: the side and each of its slots are
measured, and a buffer displayed there later gets the size back.  A
toggle, a killed buffer and a move from slot to slot therefore leave the
layout as you set it.

The sizes belong to the tab they were measured in, so each tab keeps its
own; without tabs they belong to the frame.  A new tab starts from the
options above, a tab you close forgets its sizes, and nothing is kept
across sessions.  Nil forgets them altogether, and every side window is
made with the size its option names.
