...
A buffer can also name its side itself, in =auto-side-windows-side=.  A
file that sets it as a file-local variable always opens on that side,
...
The display function writes the variable too, so a buffer that went to a
side goes back to the same one.  Nil leaves the buffer to the rules.
...
| =auto-side-windows-{top,bottom,left,right}-alist=             | alist         | the action alist of that side; the size belongs above |
...
Put the size here and not in the action alist of the side.  Both are
read, and this option is the one that wins; a =window-width= or a
=window-height= left in an =auto-side-windows-*-alist= applies only
where the option of the side is empty.
...
own, and each frame keeps its own tabs.  This works whether or not
=tab-bar-mode= is on, because Emacs keeps a current tab either way.  A
new tab starts from the options above, a tab you close forgets its sizes,
and nothing is kept across sessions.
...
A caller can name the slot itself, because the rules of this package are
display actions: a =(slot . N)= in the action alist of a =display-buffer=
call puts the buffer in slot =N=, whatever the rules would have picked.
The search for a free slot counts up from zero, so a negative slot is one
no other buffer is ever given.  =dirvish-side= asks for slot -1 and keeps
it to itself that way, without a rule of ours.
...
- The package answers for each buffer that Emacs displays, because
  ...
  that matches no rule costs 50 to 60 microseconds.
...
the slot, the geometry that keeps the sizes of a tab, the swap behind
the move commands and the drag, and the toggle.
